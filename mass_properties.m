...
Js_S_B = Js_B_B - ms * ((dot(rsb_B, rsb_B) * eye(3,3)) - (rsb_B * rsb_B')); % kg*m^2 
...
Jd_D_B = Jd_B_B - md * ((dot(rdb_B, rdb_B) * eye(3,3)) - (rdb_B * rdb_B')); % kg*m^2 
...
rcs_B = rcb_B - rsb_B;
Js_C_B = Js_S_B + ms * ((dot(rcs_B, rcs_B) * eye(3,3)) - (rcs_B * rcs_B')); % kg*m^2
...
rcd_B = rcb_B - rdb_B;
Jd_C_B = Jd_D_B + md * ((dot(rcd_B, rcd_B) * eye(3,3)) - (rcd_B * rcd_B')); % kg*m^2
...
[V,D] = eig(J_C_B);
e11_B = V(:,1);
e1_B = e11_B / norm(e11_B);
e21_B = V(:,2);
e2_B = e21_B / norm(e21_B);
e31_B = cross(e1_B, e2_B);
e3_B = e31_B / norm(e31_B);
